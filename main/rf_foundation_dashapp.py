...
from main.run_rf_foundation import *
...
controls = html.Div(
    ...
    children=[
        ...
        html.Div(
            [
                html.Label("Foundation width [m]", style={"marginBottom": "4px"}),
                dcc.Slider(
                    id="foundation-width",
                    min=1,
                    max=50,
                    step=None,
                    value=10,
                    marks={v: str(v) for v in [1, 5, 10, 20]},
...
@app.callback(
    ...
    Input("go", "n_clicks"),
    State("foundation-width", "value"),
    State("theta-x", "value"),
    State("theta-y", "value"),
    State("random-seed", "value"),
    prevent_initial_call=True,
)
def compute_and_show(n_clicks, width, tx, ty, random_seed):
    ...
    return src, f"Rendered with horizontal θ={tx:.2f}, vertical θ={ty:.2f}, width={int(width)}."
